...
[Binding]
public class DailyConsolidatedReportStepDefinitions : IClassFixture<TestWebApplicationFactory<Program>>
{
    ...
    private DailyConsolidated[] GenerateMassOfData(DateOnly date, decimal totalCredits, decimal totalDebits)
        =>
        [
    ...
    public void GivenDateAsAConsultationDate(string date)
    {
        _consultationDate = date.Equals("today", StringComparison.InvariantCultureIgnoreCase) ?
            DateOnly.FromDateTime(DateTime.UtcNow).ToString() : date;
    }
    ...
    // [Given("there are no entries for the date {string}")]
    // public void GivenThereAreNoEntriesForTheDate(string date)
    // {
    //     _consultationDate = date;
    // }

    // [Given("I provide an invalid consultation date {string}")]
    // public void GivenIProvideAnInvalidConsultationDate(string date)
    // {
    //     _consultationDate = date;
    // }
    ...
    // [Then("the value of the IsClosed property must be {bool}")]
    // public void ThenTheValueOfTheIsClosedPropertyMustBe(bool shouldBeClosed)
    // {
    //     if (_dailyConsolidatedExpected is null)
    //         throw new InvalidOperationException("DailyConsolidatedExpected is not set. Please check the table data.");

    //     _dailyConsolidatedExpected.IsClosed.Should().Be(shouldBeClosed);
    // }
